...
resource "jamfpro_category" "category_ios26_cis_benchmarks" {
...
resource "jamfpro_smart_mobile_device_group" "group_ios26" {
...
resource "jamfpro_mobile_device_configuration_profile_plist" "config_ios26" {
  ...
  level              = "Device Level"
  redeploy_on_update = "Newly Assigned"
  category_id        = jamfpro_category.category_ios26_cis_benchmarks.id
  ...
  scope {
    all_mobile_devices      = false
    mobile_device_group_ids = [jamfpro_smart_mobile_device_group.group_ios26.id]
  }
}
