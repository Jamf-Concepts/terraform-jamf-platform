...
1. Sign in to [account.jamf.com](https://account.jamf.com) and open
   **Integrations**.
2. Create an integration with the privileges required. Use broad access while
   learning; tighten later.
3. Generate a **Client ID** and **Client Secret** for the integration.
4. Note the **Tenant ID** — click the tenant pill in the integration details
   panel at account.jamf.com to reveal the tenant UUID.
5. Pick the **API gateway URL** for your region:
   - US — `https://us.apigw.jamf.com`
   - EU — `https://eu.apigw.jamf.com`
   - APAC — `https://apac.apigw.jamf.com`

The Client ID, Client Secret, Tenant ID, and base URL go in `terraform.tfvars`.
...
**`modules/jamfplatform/`** contains the actual Jamf resource definitions —
...
- **One env folder per Jamf tenant.** Each environment folder holds only
  what is genuinely tenant-specific: which API gateway to talk to, which
  integration credentials and tenant ID to use, where state lives, and which
...
- **Two ways to handle environment-specific differences.** Resources that
  are common but configured slightly differently (e.g. a device group with
  ...
  environment (e.g. a debug-only device group in dev) are defined directly
...
credentials, and tenant ID. This file is gitignored and must never be
committed.
...
`-parallelism=1` is required. The Jamf API rate-limits concurrent
...
tenant-specific — each Jamf environment has its own.
...
`modules/jamfplatform`. The module never sees the file path — only the content.
...
file in `modules/jamfplatform/` directly. Resources follow the `for_each`-over-locals
...
| `provider.tf` | No change needed if both tenants are in the same region; the gateway URL, credentials, and tenant ID come from `terraform.tfvars`. |
| `terraform.tfvars` | Gateway URL, integration credentials, tenant ID, and token paths for the production tenant. |
...
intentionally aligned with jamformer's output so the structural refactor is a
...
Option 2 is generally easier for large exports because you skip the
...
manually. It is not required for any Jamf functionality and can be removed
along with `mac_app_store_apps.tf` and `mobile_device_apps.tf` if preferred.

Provider version constraints are declared in `modules/jamfplatform/terraform.tf`
(and `environments/dev/terraform.tf` for the root module). Run
...
`client_secret`, and `tenant_id` belong to the same Jamf Account integration.
A tenant ID from a different region will fail against the wrong gateway.
...
resource in the Jamf UI. Either revert the manual change in the UI or
...
- [Managing Jamf configuration with Terraform: an introduction](https://concepts.jamf.com/guides/infrastructure-as-code/managing-jamf-configuration-with-terraform-an-introduction/) — hands-on walkthrough
